...
task('gelato', 'Prints the list of accounts').setAction(async ({}, hre) => {
...
//   let [canExec, payload] = await partyApp.checker();

//     console.log(69,canExec);
...
execAddress = partyApp.address;
execSelector = await  ethers.utils.defaultAbiCoder.encode(['string'],["startPartyandCancel"]);
resolverAddress = partyApp.address;
resolverData = await partyApp.interface.encodeFunctionData("checkerCancel",[deployerAddress]);
